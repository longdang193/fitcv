...
Source: docs/operating_system/agent_memory/patterns.md
...
## Run GitNexus in WSL with native Node, not Windows interop npx

- When using WSL for GitNexus reliability, first confirm `which npx` resolves to a Linux path, not `/mnt/c/Program Files/nodejs/npx`.
- In WSL sessions, load `nvm` and pin Node 22 before GitNexus operations:
  - `export NVM_DIR="$HOME/.nvm"`
  - `source "$NVM_DIR/nvm.sh"`
  - `nvm use 22 >/dev/null`
- Prefer installed WSL `gitnexus` `1.6.4-rc.73` on this machine because it restores healthy `query` behavior (no read-only FTS ensure failures seen in `1.6.3`).
- Run `gitnexus analyze -v` for first-pass validation, then `gitnexus status` to confirm `Status: up-to-date`.

## Keep Python files indexable for GitNexus deep graph usage

- Do not keep `*.py` in `.gitnexusignore` as a long-lived workaround; it removes Python symbol extraction and makes `context`/`impact` effectively useless for Python code.
- If Python scope extraction warnings appear, prefer partial indexing over zero Python coverage, then continue source-first for affected symbols.
- Use the repo wrapper `scripts/gitnexus_wsl.ps1` for stable execution from Windows terminals:
  - `powershell -ExecutionPolicy Bypass -File scripts/gitnexus_wsl.ps1 status`
  - `powershell -ExecutionPolicy Bypass -File scripts/gitnexus_wsl.ps1 context run_pipeline -r fitcv`
  - `powershell -ExecutionPolicy Bypass -File scripts/gitnexus_wsl.ps1 impact run_pipeline -r fitcv --depth 2`
  - `powershell -ExecutionPolicy Bypass -File scripts/gitnexus_wsl.ps1 query run_pipeline -r fitcv -l 3`
- When Windows `npx gitnexus ...` deep commands fail silently, treat the wrapper path as canonical for this host.
