...
def _persist_run_initial(run: PipelineRun, *, client: Any, project: str, dataset: str) -> None:
    if _CP_STORE is not None:
        ...
        return
    insert_run(run, client, project=persistence_project_id, dataset=persistence_dataset_id)
...
def _persist_run_queue_job_id(
    ...
    *,
    ...
    if _CP_STORE is not None:
        ...
        return
    update_run_queue_job_id(run_id, queue_job_id, client, project=persistence_project_id, dataset=persistence_dataset_id)
...
def _persist_stage_artifacts_terminal_snapshot(
    *,
    ...
    persistence_project_id = project
    persistence_dataset_id = dataset
    run = get_run(run_id, client, project=persistence_project_id, dataset=persistence_dataset_id)
...
def _finalize_review_draft_as_cv_artifact(
    *,
    ...
    errors = insert_cv_version_row(version_record, client, project=persistence_project_id, dataset=persistence_dataset_id)
    if errors:
...
def _persist_post_hitl_closure_artifact_reconciliation(
    *,
    ...
    fresh_run = get_run(run_id, client, project=persistence_project_id, dataset=persistence_dataset_id) or run
...
def _build_enriched_tab_context(
    ...
    *,
    ...
    enriched_jobs = list_run_structured_jobs(run_id, client, project=persistence_project_id, dataset=persistence_dataset_id)
    ...
    filter_results = list_filter_results_for_run(run_id, client, project=persistence_project_id, dataset=persistence_dataset_id)
    ...
    *,
    ...
    def _reconcile_orphaned_running_run(run: PipelineRun) -> PipelineRun:
        ...
        if run_age_seconds >= 300 and not completed_stages:
            events = get_events(run.run_id, client, project=persistence_project_id, dataset=persistence_dataset_id)
            if not events:
                update_run_status(
                    run.run_id,
                    RunStatus.FAILED,
                ...
                return get_run(run.run_id, client, project=persistence_project_id, dataset=persistence_dataset_id) or run
    ...
    def _build_settings_context(active: dict[str, Any], **extra: Any) -> dict[str, Any]:
        ...
        def _resolve_mode_summary() -> dict[str, str]:
            agentic_enabled = bool(effective.get("cv.agentic_late_stage.enabled"))
            runtime_expectation = _resolve_live_langgraph_runtime_expectation()
            runtime_provider = str(runtime_expectation.get("provider") or "").strip()
            runtime_model = str(runtime_expectation.get("model") or "").strip()
            runtime_source = str(runtime_expectation.get("source") or "").strip()
            has_runtime_expectation = bool(runtime_provider or runtime_model)
            authority_state = "aligned"
            drift_reason = ""
            if not agentic_enabled and runtime_source == "env_override":
                authority_state = "drifted"
                drift_reason = "Agentic runtime env is configured but agentic mode toggle is OFF."
            elif agentic_enabled and not has_runtime_expectation:
                authority_state = "drifted"
                drift_reason = "Agentic mode toggle is ON but resolved LangGraph runtime is missing."
            return {
        ...
        if mode_summary["runtime_provider"] != "—" or mode_summary["runtime_model"] != "—":
            runtime_source_label = "env-managed" if mode_summary["runtime_source"] == "env_override" else "control-plane-managed"
            agentic_runtime_note = (
                f"Agentic live runtime is {runtime_source_label}"
    ...
    def _execute_trigger(
        ...
        *,
        ...
        _persist_run_initial(run, client=client, project=persistence_project_id, dataset=persistence_dataset_id)
        submission = submit_run(
            jobs_path=actual_jobs_path,
            config_path=config_path,
            triggered_by=triggered_by,
    ...
    def get_runs_list() -> list:
        runs = list_runs(client, project=persistence_project_id, dataset=persistence_dataset_id)
    ...
    def get_run_events_list(run_id: str) -> list:
        ...
        events = get_events(run_id, client, project=persistence_project_id, dataset=persistence_dataset_id)
        return [
            {
                "event_id": e.event_id,
                "stage": e.stage,
    ...
    def admin_runs(request: Request) -> HTMLResponse:
        ...
        if view == "archived":
            runs = list_runs(client, project=persistence_project_id, dataset=persistence_dataset_id, archived_only=True)
        elif view == "all":
            runs = list_runs(client, project=persistence_project_id, dataset=persistence_dataset_id, include_archived=True)
        else:  # default: active
            runs = list_runs(client, project=persistence_project_id, dataset=persistence_dataset_id, include_archived=False)
        runs = [_reconcile_orphaned_run(run) for run in runs]
    ...
    def admin_stop_run(run_id: str) -> dict:
        ...
        if run.status == RunStatus.QUEUED and run.queue_job_id:
            ...
            if cancelled_in_queue:
                ...
                request_run_cancel(run_id, "admin", RunStatus.CANCELLED.value, client, project=persistence_project_id, dataset=persistence_dataset_id)
                append_event(
                    RunEvent(
                    ...
                    client, project=persistence_project_id, dataset=persistence_dataset_id,
                )
                append_event(
                    RunEvent(
        ...
        if run.status == RunStatus.QUEUED and run.started_at is None:
            request_run_cancel(run_id, "admin", RunStatus.CANCELLING.value, client, project=persistence_project_id, dataset=persistence_dataset_id)
            append_event(
                RunEvent(
                    run_id=run_id, event_id=event_id, stage="cancel_requested",
    ...
    def admin_bulk_cancel_runs(payload: BulkRunActionRequest) -> dict[str, Any]:
        ...
        for run_id in payload.run_ids:
            ...
            request_run_cancel(run_id, "admin", target_status, client, project=persistence_project_id, dataset=persistence_dataset_id)
            append_event(
                RunEvent(
                    run_id=run_id,
                    event_id=str(uuid.uuid4()),
    ...
    def admin_bulk_archive_runs(payload: BulkRunActionRequest) -> dict[str, Any]:
        ...
        for run_id in payload.run_ids:
            ...
            archive_run(run_id, "admin", client, project=persistence_project_id, dataset=persistence_dataset_id)
            append_event(
                RunEvent(
                    run_id=run_id,
                    event_id=str(uuid.uuid4()),
    ...
    def admin_bulk_unarchive_runs(payload: BulkRunActionRequest) -> dict[str, Any]:
        ...
        for run_id in payload.run_ids:
            ...
            unarchive_run(run_id, client, project=persistence_project_id, dataset=persistence_dataset_id)
            append_event(
                RunEvent(
                    run_id=run_id,
                    event_id=str(uuid.uuid4()),
    ...
    def admin_bulk_delete_archived_runs(payload: BulkDeleteArchivedRunsRequest) -> dict[str, Any]:
        result = delete_archived_runs(payload.older_than_days, client, project=persistence_project_id, dataset=persistence_dataset_id)
    ...
    def admin_continue_run(request: Request, run_id: str) -> dict:
        ...
        update_run_status(run.run_id, RunStatus.QUEUED, client, project=persistence_project_id, dataset=persistence_dataset_id)
        ...
        try:
            ...
            update_run_status(run.run_id, RunStatus.AWAITING_CONTINUE, client, project=persistence_project_id, dataset=persistence_dataset_id)
            update_run_checkpoint(
                run.run_id,
                client,
                project=persistence_project_id,
        ...
        _persist_run_queue_job_id(run.run_id, submission.queue_job_id, client=client, project=persistence_project_id, dataset=persistence_dataset_id)
    ...
    def admin_retry_run(run_id: str) -> dict[str, Any]:
        ...
        store = _resolve_run_store(client, project=persistence_project_id, dataset=persistence_dataset_id)
    ...
    def admin_run_detail(request: Request, run_id: str) -> HTMLResponse:
        ...
        cv_versions = list_cvs_for_run(run_id, client, project=persistence_project_id, dataset=persistence_dataset_id)
        results_rows = _results_export_rows(run)
    ...
    async def admin_run_synonym_proposals_batch_action(
        ...
        if applied == 0 and failed == 0 and len(deduped_decisions) > 0 and skipped == len(deduped_decisions):
            recent_noop_guard_exists = any(
                ...
                for event in get_events(run.run_id, client, project=persistence_project_id, dataset=persistence_dataset_id)[-10:]
            )
    ...
    def download_cv(version_id: str):
        content = get_cv_markdown(version_id, client, project=persistence_project_id, dataset=persistence_dataset_id)
        if content is None:
    ...
    def download_run_artifact_bundle_zip(run_id: str) -> Response:
        ...
        if run.status == RunStatus.SUCCEEDED:
            ...
            for cv in cv_versions:
                ...
                if not version_id:
                    continue
                cv_markdown = get_cv_markdown(version_id, client, project=persistence_project_id, dataset=persistence_dataset_id)
                if not cv_markdown:
                    continue
