...
from fitcv_cp.app_run_support import (
    _TRUNCATED_MARKDOWN_SENTINELS,
    _build_cv_generation_failure_reason_summary,
    _build_cv_generation_review_required_payload,
    _build_ranked_cv_outcome_summary,
    _extract_unsupported_requirements,
    _fit_classification_badge_class,
    _format_compact_utc_timestamp,
    _is_hitl_resolution_pending,
    _is_hitl_review_pending_state,
    _load_run_cv_generation_debug_payload,
    _map_review_required_reason_code,
    _normalize_hitl_resolution_status,
    _normalized_cv_debug_payload_for_export,
...
def _control_plane_stage_specs() -> tuple[ControlPlaneStageSpec, ...]:
    return tuple(
        ControlPlaneStageSpec(
            stage_id=stage_id,
            artifact_filename=stage_artifact_filename(stage_id),
            download_label=stage_download_label(stage_id),
        )
        for stage_id in BUNDLE_STAGE_IDS
    )


def _control_plane_stage_ids_with_reuse_metrics() -> tuple[str, ...]:
    late_stage_ids = {"enrich", "ranking", "cv_analysis", "cv_generation"}
    return tuple(spec.stage_id for spec in _control_plane_stage_specs() if spec.stage_id in late_stage_ids)


def _control_plane_bundle_artifact_specs() -> tuple[ControlPlaneArtifactSpec, ...]:
    stage_specs = _control_plane_stage_specs()
    return (
        ControlPlaneArtifactSpec("results.json", "Results JSON (Job Ledger)", "/admin/runs/{run_id}/export.json", "results"),
        ControlPlaneArtifactSpec("hitl-review-audit.json", "HITL Review Audit JSON", "/admin/runs/{run_id}/hitl-review-audit.json", "hitl_review"),
        ControlPlaneArtifactSpec("stage-artifacts.json", "Stage Artifacts JSON (Diagnostics)", "/admin/runs/{run_id}/stage-artifacts.json", "stage_artifacts"),
        *tuple(
            ControlPlaneArtifactSpec(
                spec.artifact_filename,
                spec.download_label,
                f"/admin/runs/{{run_id}}/stage-artifacts/{spec.stage_id}.json",
                "stage_slice",
        ...
        ControlPlaneArtifactSpec("settings-used.json", "Settings Used JSON", "/admin/runs/{run_id}/settings-used.json", "settings_used"),
        ...
        ControlPlaneArtifactSpec("synonym-suppression-diff.json", "Synonym Suppression Diff JSON", "/admin/runs/{run_id}/synonym-suppression-diff.json", "synonym_trace"),
        ControlPlaneArtifactSpec("approved-synonym-proposals.yaml", "Approved Synonym Overlay Delta YAML", "/admin/runs/{run_id}/approved-synonym-proposals.yaml", "approved_overlay"),
        ControlPlaneArtifactSpec("synonym-overlay-used.yaml", "Synonym Overlay Delta Used YAML", "/admin/runs/{run_id}/settings-used.json", "overlay_used"),
    )


def _control_plane_bundle_artifact_spec_map() -> dict[str, ControlPlaneArtifactSpec]:
    return {spec.filename: spec for spec in _control_plane_bundle_artifact_specs()}


def _build_run_artifact_file(
    run_id: str,
    filename: str,
    content: str,
    *,
    show_in_exports: bool | None = None,
) -> RunArtifactFile:
    spec = _control_plane_bundle_artifact_spec_map()[filename]
    return RunArtifactFile(
        filename=filename,
        label=spec.label,
        href=spec.href_template.format(run_id=run_id),
        content=content,
        show_in_exports=spec.show_in_exports if show_in_exports is None else show_in_exports,
    )
...
def _build_available_run_artifact_files(run: PipelineRun) -> list[RunArtifactFile]:
    ...
    artifact_payloads = build_terminal_run_artifact_payloads(run_record=run, events=[])

    results_payload = artifact_payloads.get("export.json")
    if run.status == RunStatus.SUCCEEDED and isinstance(results_payload, dict):
        files.append(
            _build_run_artifact_file(
                run.run_id,
                "results.json",
                _json.dumps(results_payload, ensure_ascii=False, indent=2),
            )
        )
    hitl_review_payload = artifact_payloads.get("hitl-review-audit.json")
    if run.status == RunStatus.SUCCEEDED and isinstance(hitl_review_payload, dict):
        files.append(
            _build_run_artifact_file(
                run.run_id,
                "hitl-review-audit.json",
                _json.dumps(hitl_review_payload, ensure_ascii=False, indent=2),
            )
        )
    if run.status == RunStatus.SUCCEEDED and run.cv_generation_debug_json:
        files.append(
            _build_run_artifact_file(
                run.run_id,
                "cv-debug.json",
                pretty_json_string_or_fallback(run.cv_generation_debug_json),
            )
        )
        ...
        if isinstance(review_required_payload, dict):
            files.append(
                _build_run_artifact_file(
                    ...
                    _json.dumps(review_required_payload, ensure_ascii=False, indent=2),
                )
            )
    ...
    if run.status == RunStatus.SUCCEEDED and run.settings_used_json:
        files.append(
            _build_run_artifact_file(
                run.run_id,
                "settings-used.json",
                pretty_json_string_or_fallback(run.settings_used_json),
            )
        )
    if run.mapping_suggestions_json and _run_has_reached_stage(run, "enrich") and "enrich" in stage_artifacts_by_id:
        files.append(
            _build_run_artifact_file(
                ...
                pretty_json_string_or_fallback(run.mapping_suggestions_json),
            )
        )
    if run.synonym_proposals_json and (
        ...
        files.append(
            _build_run_artifact_file(
                ...
                pretty_json_string_or_fallback(run.synonym_proposals_json),
            )
        )
    ...
    if runtime_overlay_yaml:
        files.append(
            _build_run_artifact_file(
                ...
                runtime_overlay_yaml if runtime_overlay_yaml.endswith("\n") else runtime_overlay_yaml + "\n",
            )
        )
    synonym_proposals_trace_payload = artifact_payloads.get("synonym-proposals-trace.json")
    synonym_suppression_diff_payload = artifact_payloads.get("synonym-suppression-diff.json")
    if isinstance(synonym_proposals_trace_payload, dict) and isinstance(synonym_suppression_diff_payload, dict):
        files.append(
            _build_run_artifact_file(
                run.run_id,
                "synonym-proposals-trace.json",
                _json.dumps(synonym_proposals_trace_payload, ensure_ascii=False, indent=2),
            )
        )
        files.append(
            _build_run_artifact_file(
                ...
                _json.dumps(synonym_suppression_diff_payload, ensure_ascii=False, indent=2),
            )
        )
    if stage_transition_artifact_payload and run.status != RunStatus.QUEUED:
        files.append(
            _build_run_artifact_file(
                ...
                _json.dumps(stage_transition_artifact_payload, ensure_ascii=False, indent=2),
            )
        )
        for stage_id in BUNDLE_STAGE_IDS:
            ...
            files.append(
                _build_run_artifact_file(
                    run.run_id,
                    stage_artifact_filename(stage_id),
...
def _finalize_review_draft_as_cv_artifact(
    *,
    ...
    version_record = create_cv_version_record(
        ...
        final_score=float(
            row.get("baseline_fit")
            if row.get("baseline_fit") is not None
            else row.get("final_score") or 0.0
        ),
...
def _artifact_applicability_state(run: PipelineRun, filename: str, included_files: set[str]) -> str:
    if filename in included_files:
        return "present"
    spec = _control_plane_bundle_artifact_spec_map().get(filename)
    if spec is None:
        return "missing"
    if spec.state_kind == "stage_slice":
    ...
    if spec.state_kind == "synonym_trace":
        ...
        return "missing"
    if spec.state_kind == "approved_overlay":
        payload = _load_run_synonym_proposals_payload(run)
        if not isinstance(payload, dict):
            return "not_applicable"
        proposals = [item for item in list(payload.get("proposals") or []) if isinstance(item, dict)]
    ...
    if spec.state_kind == "overlay_used":
...
def create_app(
    *,
    ...
    async def admin_settings_update_key(request: Request, key: str) -> HTMLResponse:
        ...
        try:
            save_setting(canonical_key, coerced, updated_by="admin")
        except RuntimeError as exc:
            active = load_active_settings()
            return templates.TemplateResponse(
                request=request,
    ...
    def download_run_hitl_review_audit_json(run_id: str) -> Response:
        ...
        payload = build_terminal_run_artifact_payloads(run_record=run, events=[]).get("hitl-review-audit.json")
        if not isinstance(payload, dict):
            raise HTTPException(status_code=404, detail="HITL review audit export is not available for this run")
        return Response(
    ...
    def download_run_synonym_proposals_trace_json(run_id: str) -> Response:
        ...
        trace_payload = build_terminal_run_artifact_payloads(run_record=run, events=[]).get("synonym-proposals-trace.json")
        if not isinstance(trace_payload, dict):
    ...
    def download_run_synonym_suppression_diff_json(run_id: str) -> Response:
        ...
        payload = build_terminal_run_artifact_payloads(run_record=run, events=[]).get("synonym-suppression-diff.json")
        if not isinstance(payload, dict):
