...
excludes:
  - control-plane sqlite persistence for cv_versions
tags:
  - fast
  - ci-safe
"""
...
def test_store_cv_version_uses_control_plane_store(tmp_path, monkeypatch) -> None:
    ...
    rows = sqlite_store.list_cvs_for_run("rid-sqlite", None, project="", dataset="")
    matched = [row for row in rows if row.get("version_id") == record["version_id"]]
