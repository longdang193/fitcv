...
| Bookmarks | `renderBookmarksPage` | `src/fitcv_cp/templates/bookmarks.html` | Bookmark routes and SQLite store | Follow-up aligns page shell, all seven stage tabs, grouped jobs table, conditional selection panel, page-size controls, stage-aware export naming, canonical async failure ownership, rating/CV row actions, and Run Details stage/CV projections. | already aligned | `tests/test_fitcv_cp/test_app.py`, `tests/test_fitcv_cp/test_store.py`, `tests/test_fitcv_cp/test_sqlite_store.py` |
| Synonyms | `renderSynonymsPage`, `renderSynonymDetails` | `src/fitcv_cp/templates/synonyms.html` | Synonym routes, policy IO, SQLite store | Follow-up aligns Workspace header/actions, collapsible sections, editor find-next/cancel controls, review table columns/select-all, tab counts, confidence/candidate/evidence fields, occurrence metadata, evidence pagination totals, and structured collapsed processing log. | already aligned | `tests/test_fitcv_cp/test_app.py`, Synonym policy/preview/commit tests |
| Preference Optimization | list/detail optimization renderers | `src/fitcv_cp/templates/optimization.html` | Optimization routes/service/settings/store | Follow-up aligns prototype empty-state guidance, fallback notice class, canonical ranking-mode selector identity, and PRG stale/retry notice classification. Stale actions reload current state; they do not replay candidate creation. | change narrowly | `tests/test_fitcv_cp/test_optimization_page.py`, `tests/test_inverse_optimization.py`, `tests/test_fitcv_cp/test_sqlite_store.py` |
...
- Bookmark and Synonym contract gaps are closed. Preference Optimization
  ...
  stale and retryable actions explicitly reload current state rather than
  pretending to retry candidate creation. Bookmark CV availability now follows
  the current CV version. Product owner approval remains required for UX Freeze.
