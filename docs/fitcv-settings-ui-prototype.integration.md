...
|---|---|---|---|---|---|---|
| Shared shell | prototype shell/navigation/header/shared CSS | `src/fitcv_cp/templates/base.html` | shared `fitcvApiRequest`, canonical error handlers | Task 1 removed visible `Lifecycle`, retained direct route, added shared async rendering and duplicate-submit lock, and removed duplicate `.global-header` ownership; fresh validator still owns final mobile/theme/focus and shared-selector proof | change narrowly | `tests/test_fitcv_cp/test_local_routes.py`, `tests/test_fitcv_cp/test_app.py` |
