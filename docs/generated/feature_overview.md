...
| `inspection_debugging` | 2.2.0 | add | fitcv_cp | Admins can see what the pipeline did for a run, including selectable rule-filter marks, stage-local artifacts, inputs used, and generated CVs. |
...
| `trigger_run_management` | 2.2.0 | add | fitcv_cp | Admins can trigger runs via path reference, file upload, or pasted JSON, run the pipeline automatically or stage by stage, and configure which deterministic rule-filter checks block versus only mark jobs. |
