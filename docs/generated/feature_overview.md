...
| `cv_system` | 1.21.0 | add | fitcv | CV generation uses a structured preset-based configuration, hybrid lexical-plus-semantic `cv_analysis` evidence alignment, richer shortlist retrieval inputs and reuse, analysis-grounded final-stage validation, stage-level quality metrics, and exact-match late-stage reuse for ranking AI-score rows and `cv_analysis` records. |
| `inspection_debugging` | 2.13.0 | add | fitcv_cp | Admins can see what the pipeline did for a run, including shortlist query/reuse diagnostics, ranking contribution visibility, lexical-vs-semantic `cv_analysis` detail, compact stage-level quality metrics, late-stage reuse diagnostics, stage-local artifacts, and hybrid validation provenance from deterministic and soft-claim grounding checks. |
