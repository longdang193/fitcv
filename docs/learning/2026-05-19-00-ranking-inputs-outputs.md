---
aliases: []
status: []
time: 2026-05-19-00-29-33-44
tags:
  - "#interview-prep"
  - "#pipeline-stage"
  - "#ANKI"
TARGET DECK: FITCV::PIPELINE-STAGES
question_type: qa
learning_mode: interview
bloom_level: auto
source_scope: repo
---

## ranking - inputs-outputs

SSTART

T-F_Obsidian-v2

Q: ranking stage - Inputs and outputs: What information enters this stage, and what should come out?

A: In: scoring shortlist + reranker outputs + six-feature inputs. Out: ranked jobs, fit labels, reuse/quality metrics, artifacts.

E: Source-grounded fact: See docs/stages/ranking.yaml for this stage contract. Interview explanation: Explain why this decision exists, tradeoffs it introduces, and downstream impact.
<!--ID: 1779144736134-->
EEND


