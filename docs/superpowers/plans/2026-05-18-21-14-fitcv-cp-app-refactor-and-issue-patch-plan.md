...
**Steps:**
- [x] Step 1: capture baseline with `uvx pytest tests/` and `uvx mypy src --show-error-codes`.
- [x] Step 2: run `gitnexus_context` and `gitnexus_impact(direction="upstream")` for first-wave symbols:
  - `_timeline_semantic_outcome`
  - `get_queue_job_status`
  - `orchestration_job_status`
- [x] Step 3: record expected affected callers/processes and risk level for each symbol.

**Verification:**
- [x] baseline test/type command outputs captured.
- [x] GitNexus impact outputs captured for each first-wave symbol.
...
**Steps:**
- [x] Step 1: collapse duplicate `_timeline_semantic_outcome` into single canonical function.
- [x] Step 2: preserve accepted stage alias coverage (`layer4_cv_validation_failed` and `cv_validation_failed`).
- [x] Step 3: add/adjust focused tests asserting expected outcome mapping for representative payload combinations.

**Verification:**
- [x] targeted tests for timeline semantic outcome pass.
- [x] no second definition remains in `app.py`.
...
**Steps:**
- [x] Step 1: introduce SSOT helper for run retrieval + standardized 404 handling contract per route type (HTML vs JSON).
- [x] Step 2: consolidate queue/orchestration status accessor into one canonical helper name and call path.
- [x] Step 3: migrate selected route families first, then remaining equivalent callsites.

**Verification:**
- [x] representative HTML and JSON routes preserve expected not-found semantics.
- [x] no divergent status accessor helpers remain.
...
**Steps:**
- [x] Step 1: extract canonical settings mutation flow (`coerce -> validate -> save`) used by single-key/group/section handlers.
- [x] Step 2: extract shared synonym overlay parser/validator (UTF-8 decode, YAML parse, scope validate).
- [x] Step 3: update route handlers to delegate through extracted flow while preserving response contract and status codes.

**Verification:**
- [x] settings route matrix passes (single-key/group/section).
- [x] overlay input matrix passes (missing file, empty bytes, non-UTF8, invalid YAML, invalid scope).
...
**Steps:**
- [x] Step 1: run full regression gates: `uvx pytest tests/` and `uvx mypy src --show-error-codes`.
- [x] Step 2: run `gitnexus_detect_changes()` and compare affected symbols/processes to planned scope.
- [x] Step 3: if drift exceeds planned scope, split/rollback last slice and re-run checks.

**Verification:**
- [x] tests and type checks pass (lane-closeout exception approved for out-of-lane baseline failures; see execution context pack evidence).
- [x] GitNexus detect-changes output matches expected bounded scope (lane-scoped staged diff evidence captured; out-of-lane scope drift excluded).
