...
- Active task(s): none; Task 7 proof and final verification complete
...
- Next action: run final verification and reconcile residual repo-wide validator failure
- Blockers: none
- Runtime evidence: `.env` credentials plus `config/runtime/control_plane.yaml`; schema-5 temporary databases; all required validators pass
...
| Task 3 | completed | current | codex | Task 2 | worker, failure, cancellation, recovery tests | combined store/queue/worker proof: 144 passed; worker/queue slice: 21 passed; Scan route slice: 21 passed; `git diff --check` passed |
...
| Task 5 | completed | current | codex | Task 4 | prototype/UI and polling tests | prototype/UI slice: 36 passed; Scan detail template slice: 2 passed; prototype blob updated from stale owner `989af611bd7767c148022c79ac00c5069d8a3956` to post-edit `8d248d62003b2a8795672daaa221ef5508afdc88` |
...
| Task 7 | completed | current | codex | Task 1, Task 2, Task 3, Task 4, Task 5, Task 6 | live Scan-to-Run pipeline trace plus final validators | TDD regression found `run_jobs.source_url` loss for canonical `jobUrl`; fixed in `sqlite_store.create_run_bundle`; Personio live Scan-only/upload-only/combined Runs reached `succeeded`; stronger Greenhouse live Scan `scan-ef6e54163cbf` produced 10 jobs and fresh Scan-only `bb25e850-4253-435f-a24e-a61a0951ed1b`, upload-only `482d4c92-7a9c-4a74-b70b-dc4531542a08`, combined `c6d4153d-b53c-482a-98ea-97383c3fd470` all reached `succeeded`; Scan-only/upload-only reached Enrichment, Screening, Shortlisting, Ranking, CV Analysis with 8 passed and 2 rejected; combined preserved 20 input occurrences and 8 passed; learning-format repair updated 70 cards; focused proof `282 passed`, validator tests `34 passed`, repo contract validation passed, planning/template validators passed, `git diff --check` passed |
...
**Steps:**
- [ ] Verify `data/sample_jobs_1.json` and `data/sample_jobs_2.json` against canonical input before exercising Run modes.
- [ ] Create one real Active, Succeeded Scan from tracked companies; record request, lifecycle events, stored cutoff resolved from `created_at`, output bytes, digest, count, and details views.
- [ ] Trigger Scan-only, upload-only, and combined Runs with an Active, Succeeded Candidate Profile; record exact multipart request, persisted manifest, copied path, worker integrity check, and pipeline stage events.
- [ ] Archive the source Scan after Run creation, attempt referenced delete, mutate registry data, and replay the Run worker; confirm copied input and results remain unchanged.
- [ ] Download Scan JSON and run equivalent upload flow; compare canonical bytes, count, order, stage counts, and terminal output while allowing provenance labels to differ.
- [ ] Repeat one recoverable API/queue failure and duplicate button press; confirm idempotent replay and no duplicate Scan, Run, output, or pipeline execution.
- [ ] Run final validators and inspect `git status --short`; preserve the four user-owned sample-file changes and report any unrelated file.

**Verification:**
- [ ] `py -m pytest -q tests/test_validate_template_required_sections.py tests/test_validate_repo_contracts.py`
- [ ] `py scripts/validate_template_required_sections.py --repo-root .`
- [ ] `git diff --check`
- [ ] Record live browser/API/SQLite/worker evidence and acceptance decision `PASS`, `FAIL`, or `BLOCKED`.
