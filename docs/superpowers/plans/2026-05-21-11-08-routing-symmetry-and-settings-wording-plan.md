...
**Steps:**
- [ ] Step 1: Add explicit section registry entries for `agentic-enablement` and `agentic-automation` with MECE key partition.
- [ ] Step 2: Keep `agentic-advanced` mapping unchanged for non-target surfaces.
- [ ] Step 3: Remove/retire implicit merged `agentic-core` save surface or keep as read-only compat alias that is not used by UI forms.
...
**Steps:**
- [ ] Step 1: Update `settings_page_sections` card `submit_slug` for Enablement to `agentic-enablement`.
- [ ] Step 2: Update `settings_page_sections` card `submit_slug` for Automation to `agentic-automation`.
- [ ] Step 3: Confirm `all_settings_sections` lookup resolves both new slugs and no card references removed slug.
...
**Steps:**
- [ ] Step 1: Add test: saving `agentic-enablement` does not change automation keys from active state.
- [ ] Step 2: Add test: saving `agentic-automation` does not change enablement keys from active state.
- [ ] Step 3: Add payload-capture assertions: each route persists only route-owned keys.

**Verification:**
- [ ] `pytest tests/test_fitcv_cp/test_app.py -k "agentic_enablement or agentic_automation or section_save"`
...
**Steps:**
- [ ] Step 1: Update descriptions for:
...
- [ ] Step 2: Add concise help block in settings template:
  - gate = permission
  - automation = policy
  - 3-state matrix (`OFF/OFF`, `ON/OFF`, `ON/ON`)
  - dependency note (`OFF/ON` blocked)
- [ ] Step 3: Keep wording symmetric across Apply and Promote flows.

**Verification:**
- [ ] Render `/admin/settings` and verify exact copy placement and no contradictory labels.
...
**Steps:**
- [ ] Step 1: Add pre-submit guardrail in settings form JS:
...
- [ ] Step 2: Prompt offers exactly two explicit actions:
  - `Enable prerequisite and continue`
  - `Keep automation off`
- [ ] Step 3: Apply choice in form state before submit; do not mutate other keys.
- [ ] Step 4: Add server-side validation mirror for direct POST safety:
  - reject inconsistent ON/OFF combination with clear error unless prerequisite key also ON in same payload.
- [ ] Step 5: Ensure behavior symmetric for Apply and Promote pathways.

**Verification:**
- [ ] `pytest tests/test_fitcv_cp/test_app.py -k "prerequisite or auto_apply or auto_promote"`
- [ ] Manual browser check: toggle auto ON with prerequisite OFF, verify two-choice prompt and deterministic result.
...
**Steps:**
- [ ] Step 1: Add/adjust section describing symmetric save boundaries by decision area.
- [ ] Step 2: Document gate/policy wording contract.
- [ ] Step 3: Document prerequisite enforcement contract: block + explicit choice, no silent auto-enable.
- [ ] Step 4: Ensure no outdated references to single `agentic-core` card-save behavior.

**Verification:**
- [ ] `rg -n "agentic-core|agentic-enablement|agentic-automation|Manual Capability Gate|Automatic Execution" docs/superpowers/specs/2026-05-04-22-20-automation-settings-run-all-contract-spec.md`
