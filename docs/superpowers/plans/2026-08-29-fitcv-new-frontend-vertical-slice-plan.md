...
**Steps:**
- [ ] Implement URL-owned Active/Archived tabs, pagination, native multi-selection, capability-driven actions, and delete preview.
- [ ] Implement New Scan with verified company selection, stable publication-window enum, idempotent submit, polling, cancel, retry, and terminal states.
- [ ] Render immutable output as Table/JSON views and expose ordered eligible Scan sources to Run trigger.

**Verification:** Existing scan suites plus frontend state tests; Playwright create/empty/error/cancel/output/reuse/delete and responsive/accessibility evidence.
...
**Steps:**
- [ ] Implement managed source picker and trigger with idempotency, active confirmed profile, and immutable input summary.
- [ ] Implement list/detail/stages/jobs, server-owned statuses/capabilities, cursor event polling, and terminal/recovery rendering.
- [ ] Implement cancel/archive/unarchive/archive-delete/debug-bundle actions with confirmation and state-specific locks.
...
**Steps:**
- [ ] Render server fit/evidence and stage/result filters; keep personal interest separate.
- [ ] Implement bookmark identity, selection preview, filtered export, remove, and dependent-removal messaging.
- [ ] Implement personalization GET/PATCH using ETag/revision, validation, conflict recovery, and truthful fallback; do not call legacy optimization HTML.
...
| Task 2 | active | `C:/Users/HOANG PHI LONG DANG/repos/JOB-PROJECT/.worktrees/fitcv-task-2` | codex | Task 1 | lifecycle API and browser journey | pending; branch `codex/fitcv-task-2`, base `75d9eb542ffa3a60760f3bbcc5fcc8030b9a7b30` |
...
| Task 2 | active | `codex/fitcv-task-2` | `75d9eb542ffa3a60760f3bbcc5fcc8030b9a7b30` / `75d9eb542ffa3a60760f3bbcc5fcc8030b9a7b30` | `frontend/src/features/candidate-profile/**`, focused frontend tests; backend only for proven canonical contract delta | pending | pending | source for Task 4 | `ui` lane active; run Task 2 proof, then accept or block |
