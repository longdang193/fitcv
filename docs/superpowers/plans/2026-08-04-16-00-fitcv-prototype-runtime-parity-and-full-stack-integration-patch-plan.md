...
- Direct approved scope removes `Lifecycle` from visible navigation. Preserve
  direct `/admin/lifecycle` compatibility routing; do not add a replacement
  sidebar item.
...
- Mode: `subagent-ready sequential`
...
- Task 1: intended `tests/test_fitcv_cp/test_local_routes.py::test_shared_shell_matches_frozen_prototype_and_canonical_error_contract`.
...
- Verify: `/admin/runs`, `/admin/scans`, `/admin/candidate-profiles`, `/admin/bookmarks`, `/admin/synonyms`, `/admin/settings`, `/admin/api-providers`, `/admin/llm-configuration`, `/admin/settings/prompt-management`, `/admin/system`, and direct `/admin/lifecycle`

**Dependencies:**
- frozen prototype hash verified
- working-tree snapshot recorded with `git status --short`
- no implementation slice active
...
**Verification:**
- [ ] `uv run pytest tests/test_fitcv_cp/test_local_routes.py tests/test_fitcv_cp/test_app.py -q -k "navigation or shell or initializer or api_error or packaged_local_pages"`
- Expected: focused shared-shell and error tests pass.
...
- Expected: exact prototype shell and no Lifecycle link; direct `/admin/lifecycle` remains reachable.
...
3. runtime navigation matches prototype, Prompt Management remains visible, Lifecycle is hidden from sidebar, direct compatibility routes remain;
