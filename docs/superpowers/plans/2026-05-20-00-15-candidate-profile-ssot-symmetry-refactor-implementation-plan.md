...
**Steps:**
- [ ] Step 1: Run baseline targeted tests for candidate/ranking/vector/evidence behavior.
- [ ] Step 2: Run GitNexus upstream impact for symbols to edit (`validate_profile`, `load_profile_yaml`, `infer_role_family`, `build_candidate_query_components`, normalization helpers).
- [ ] Step 3: Record affected callers/processes and finalize bounded patch sequence from impact output.

**Verification:**
- [ ] `uvx pytest tests/test_candidate.py tests/test_vector_search.py tests/test_ranking.py tests/test_evidence.py`
- [ ] GitNexus impact records captured for each edited symbol.
...
**Steps:**
- [ ] Step 1: Introduce canonical profile payload adapter path reused by loader entry points.
- [ ] Step 2: Add type guards for list/dict mixed-shape fields in validation and row preparation paths to eliminate `AttributeError` edges.
- [ ] Step 3: Keep required-sections + inference outputs invariant; update tests for deterministic error semantics.

**Verification:**
- [ ] `uvx pytest tests/test_candidate.py -q`
...
**Steps:**
- [ ] Step 1: Thread config through role-family inference in query component assembly.
- [ ] Step 2: Preserve existing output keys/order while aligning family hints with candidate/ranking taxonomy rules.
- [ ] Step 3: Add tests for config-aware and configless fallback parity.

**Verification:**
- [ ] `uvx pytest tests/test_vector_search.py -k "candidate_query or role_family" -q`
...
**Steps:**
- [ ] Step 1: Extract or align canonical text/list normalization boundary with strict behavior parity tests.
- [ ] Step 2: Replace divergent local helper paths where equivalent semantics are required.
- [ ] Step 3: Add parity tests proving same normalization outputs across modules for shared concept fixtures.

**Verification:**
- [ ] `uvx pytest tests/test_candidate.py tests/test_ranking.py tests/test_evidence.py -q`
- [ ] `uvx mypy src --show-error-codes`
...
**Steps:**
- [ ] Step 1: Run full targeted test suite and repo hook validator subset.
- [ ] Step 2: Run GitNexus detect-changes scope check and compare against approved plan targets.
- [ ] Step 3: Record rollback/containment notes per wave (revert order C->B->A).

**Verification:**
- [ ] `uvx pytest tests/test_candidate.py tests/test_vector_search.py tests/test_ranking.py tests/test_evidence.py`
- [ ] `python scripts/hooks/run_validator.py --fast`
- [ ] `gitnexus_detect_changes()` output aligned with expected symbol/file set.
