...
- Fresh completion verification exposed retry publication re-inserting identical immutable source blocks and failing `UNIQUE (attempt_id, ordinal)`. Shared SQLite publication now reuses identical blocks and rejects divergent retry output. Focused backend proof passes `128 passed`; the full plan-owned suite passes `763 passed, 1 skipped`, with the guarded live-key integration passing separately `1 passed`. Live retry recovered attempt `attempt_86917665580b451a8b12002943b04364` from failed revision `5` to `base_review` revision `7`; real `.env` plus `cx/gpt-5.4-mini` regeneration completed at revisions `9` and `11`. Playwright verified baseline regeneration capability, Source dialog focus return, and zero console findings; Chrome DevTools verified Candidate Details status `200`, expected section structure, and zero console findings.
