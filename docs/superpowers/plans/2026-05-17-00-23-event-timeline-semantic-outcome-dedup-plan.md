...
**Steps:**
- [ ] Step 1: Introduce semantic outcome resolver using existing payload fields (`deterministic_outcome`, `stage_owned_subreason`, stage id).
- [ ] Step 2: Define qualifier policy for `expected_rejection` vs `unexpected_failure` and integrate into timeline row message/metadata.
- [ ] Step 3: Keep fallback behavior deterministic when fields are missing (mark investigate path).

**Verification:**
- [ ] Add/adjust tests for validation-failed rows to assert expected-policy qualifier when contract fields match.
- [ ] Add test for missing/partial classification fields to assert unexpected/investigate qualifier.
...
**Steps:**
- [ ] Step 1: Define canonical fingerprint payload for dedup-eligible informational events (`synonym_proposal_triage_completed` first).
- [ ] Step 2: Implement projection-time collapse rules for equivalent consecutive rows and track repeat count.
- [ ] Step 3: Expose repeat count in timeline row context so template can display compactly.

**Verification:**
- [ ] Add test fixture with repeated equivalent triage events and assert one projected row plus repeat count.
- [ ] Add test fixture with materially changed triage payload and assert no collapse.
...
**Steps:**
- [ ] Step 1: Add alias-equivalence tests (different raw stage aliases mapped to same canonical semantic outcome).
- [ ] Step 2: Add replay-invariance test (same payload sequence projects identically across runs).
- [ ] Step 3: Add assertion that raw events source remains unmodified by projection/dedup helpers.

**Verification:**
- [ ] Run focused test module for timeline behavior.
- [ ] Inspect event export/read path to confirm dedup is view-only.
...
**Steps:**
- [ ] Step 1: Regenerate planning lineage if spec/plan graph changed.
- [ ] Step 2: Run fast validator hook and required targeted tests.
- [ ] Step 3: Capture final evidence references in PR/closeout notes.
